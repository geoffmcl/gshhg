...
@set TMP3RD=%TMPROOT%\software.x64
...
@REM echo Only one command NOPAUSE allowed
@REM goto ISERR
)
@shift
@goto RPT
...
@REM Conform to the normal build-me output
@echo Building %TMPPRJ% %DATE% %TIME%, output to %TMPLOG%
@echo Building %TMPPRJ%%DATE% %TIME% > %TMPLOG%
...
@set TMPOPTS=%TMPOPTS% -DUSE_STATIC_RUNTIME:BOOL=ON
